var quizCtrl = function ($scope, $http, helper) {
    ...
    $scope.loadQuiz = function (file) {
        $http.get(file)
         .then(function (res) {
             ...
             $scope.config = helper.extend({}, $scope.defaultConfig, res.data.config);//todo
             $scope.questions = $scope.config.shuffleQuestions ? helper.shuffle(res.data.questions) : res.data.questions;//todo
             $scope.totalItems = $scope.questions.length;
             $scope.itemsPerPage = $scope.config.pageSize;
             $scope.currentPage = 1;
             $scope.mode = 'quiz';
             if ($scope.config.shuffleOptions) //todo
    ...
    $scope.isCorrect = function (question) {
        ...
        if (item.IsAnswer) {
            if (helper.toBool(item.Selected)) {
             //base on the data above item.IsAnswer=="true" would not work since it is not a string
    ...
    //    var i = 0;
    //    question.Options.forEach(function (option, index, array) {
    //        if (helper.toBool(option.Selected) != option.IsAnswer) { //option.selected was dynamically added from html and it defined when clicked else the attribute   is undefined              
    //            result = 'wrong';                        
    //        }
